...
public class AllPage extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recycleSellAdapter.setOnItemClickListener(new RecycleSellAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View v, int pos) {  //리사이클 뷰 가 눌렸을 경우 상세 페이지로 이동
                ...
                Intent intent = new Intent(AllPage.this, DetailPage.class);
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()){           //가져오는데 성공
                    if (task.getResult().size() <= 0){          //물건이 없는 경우
                        Toast.makeText(AllPage.this, "상품 없음", Toast.LENGTH_SHORT).show();
                    }else{      //물건이 있다.
